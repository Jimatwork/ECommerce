$(document).ready(function() {
	...
	layui.use(['layer', 'table'], function() {
		...
		table.render({
			elem: '#test',
			url: getRequestIp() + 'getShopClassify' //数据接口
				,
			id: 'testReload',
			method: 'post',
			toolbar: 'true',
		...
		form.on('checkbox(status)', function(obj) {
			...
			$.ajax({
				cache: true,
				type: "POST",
				url: getRequestIp() + "updateShopClassify",
				data: formData,
				async: true,
				dataType: "json",
				processData: false,
		...
		table.on('tool(demo)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
			...
			if(layEvent === 'detail') {
				sessionStorage.setItem("shopClassifySon_id", data.id);
				layer.open({
					type: 2,
					title: '子分类',
					maxmin: true,
					...
					area: ['1150px', '550px'],
					content: 'shopClassifySon.html' // iframe的url
				});
			} else if(layEvent === 'scf_update') {
				// 修改分类
				layer.open({
					type: 1,
					title: '修改分类',
				...
				layer.confirm('确定删除此数据吗？', function(index) {
					...
					$.ajax({
						cache: true,
						type: "POST",
						url: getRequestIp() + "delShopClassify",
						data: {
							'id': data.id,
						},
						async: true,
