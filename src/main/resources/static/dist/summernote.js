	...
	var dialog = renderer.create('<div class="modal" aria-hidden="false" tabindex="-1"/>', function($node, options) {
		...
		$node.html([
			...
			(options.title ?
				...
				'      <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><img src="img/shanchu.png"/></span></button>' +
			...
			(options.footer ?
				...
				'    <div style="position: relative;" class="modal-footer"><button class="dd_dd_ss" >选择视频</button>' + options.footer + '</div>' : ''
			),
			'  </div>',
			'</div>'
		].join(''));
...
//				'<input class="note-image-input form-control" type="file" name="files" accept="image/*" multiple="multiple" />' +
				...
				'</div>';
			var footer = '<button class="xz_img" style="display: block;">选择图片<button><button href="#" class="btn btn-primary note-image-btn " >' + lang.image.insert + '</button>';
	...
	var VideoDialog = function(context) {
		...
		this.createVideoNode = function(url) {
			...
			if (url.indexOf("src") != -1) {
				var ifr = $(url).attr("src");
				var $video = $('<iframe  webkitallowfullscreen mozallowfullscreen allowfullscreen>')
					.attr('frameborder', 0)
					.attr('src', ifr)
