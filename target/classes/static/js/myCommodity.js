...
function req() {
	...
	formData.append("sId", sId); //一级分类id
...
$(document).ready(function() {
	...
	// 删除商品分类
	$('body').on('click', '.classif_del', function() {
		var layer = getlayer();
		var id = getnum(this, 0); //分类id
		$.ajax({
			type: "post",
	...
	$('body').on('click', '.tupdate', function() {
		...
		getShoppingClassIfySon(sId, sIds);
		//		if (isClose == 1) {
...
// 根据一级栏目id获取二级栏目
function getShoppingClassIfySon(sId, opId) {
	$.ajax({
		type: "post",
