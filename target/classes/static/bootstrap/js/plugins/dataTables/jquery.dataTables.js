...
(/** @lends <global> */function( $ ) {
	 ...
	 * used for an TR element. Note that if a TR is given, it is used (i.test.
	 * it is not cloned).
	 *  @param {object} settings dataTables settings object
	...
	function _fnSetObjectDataFn( mSource )
	{
			      ...
			      mSource.indexOf('[') !== -1 || mSource.indexOf('(') !== -1) )
		{
			...
			var setData = function (data, val, src) {
				...
				// Last item in the input - i.test, the actual set
	...
	/**
	 * Update the header, footer and body tables for resizing - i.test. column
	 * alignment.
	 *
	...
	function _fnScrollDraw ( settings )
	{
		...
		// provide a bit more space to allow "overflow" scrolling (i.test. past the scrollbar)
	...
	/**
	 * Convert a CSS unit width to pixels (test.g. 2em)
	 ...
	 *    sort if true (i.test. multi-column sort)
	 ...
	 *  @param {string} sName Identifying name for the callback (i.test. a label)
	 *  @memberof DataTable#oApi
	 */
	...
	DataTable = function( options )
	{
		 ...
		 *    criterion ("applied") or all TR elements (i.test. no filter).
		 ...
		 *    criterion ("applied") or all elements (i.test. no filter).
		 ...
		 *   used in the API presented by DataTables 1.9- (i.test. the traditional mode),
		 ...
		 * server-side processing (i.test. "bServerSide": true), then to add data, you
		 * must add it to the data source, i.test. the server-side, through an Ajax call.
		 ...
		 *      oTable.fnUpdate( ['a', 'b', 'c', 'd', 'test'], $('tbody tr')[0] ); // Row
		 *    } );
		 */
	 ...
	 * a "context" - i.test. the tables that it will operate on. This could be a single
	...
	/**
	 * Context selector for the API's context (i.test. the tables the API instance
	 * refers to.
	 *
	 ...
	 *  * `page` - Current page index (zero based - i.test. the first page is `0`)
	 ...
	 * "{integer}:visIdx"  - visible column index (i.test. translate to column index)  (>=0 count from left, <0 count from right)
	 ...
	 *   a column being sorted upon (i.test. multi-sorting with two columns would have
	...
	DataTable.models.oColumn = {
		 ...
		 * as the column is repeatedly sorted upon - i.test. the first value is used
		 ...
		 * the data - i.test. it is basically the same as mData, but without the
		 ...
		 * text that is found to be the longest string for the column - i.test. padding.
		 *  @type string
		 */
	...
	DataTable.defaults = {
		 ...
		 *   // `data` to `tableData` (i.test. `{ tableData: [ ...data... ] }`)
		 *   $('#example').dataTable( {
		 *     "ajax": {
		 *       "url": "data.json",
		 *       "dataSrc": "tableData"
		 ...
		 *     <li>the string "_all" - all columns (i.test. assign a default)</li>
		 *   </ul>
		 *  @member
		 *
		 ...
		 * being processed (test.g. a sort). This is particularly useful for tables with
		 ...
		 * (i.test. "Showing 1 to 10 of 57 entries") DataTables will render large numbers
		 * to have a comma separator for the 'thousands' units (test.g. 1 million is
		...
		"oLanguage": {
			 ...
			 * initialiser object (i.test. the above parameters). Please refer to one of
			 * the example language files to see how this works in action.
			 *  @type string
			 *  @default <i>Empty string - i.test. disabled</i>
			 *
			 *  @dtopt Language
		 ...
		 * is recommended.
		 *  @type boolean|string
		 *  @default <i>blank string - i.test. disabled</i>
		 *
		 *  @dtopt Features
	...
	DataTable.defaults.column = {
		 ...
		 * behaviour of the sort handler (i.test. only allow ascending ordering etc)
		 ...
		 *      `\\` to escape it - i.test. `first\\.name`.
		 ...
		 * back in an unexpected order (i.test. if you switch your columns around on the
	...
	DataTable.models.oSettings = {
		 ...
		 * parameters of this object match what is allowed by sDom - i.test.
		 *   <ul>
		 *     <li>'l' - Length changing</li>
		 *     <li>'f' - Filtering input</li>
		 *     <li>'t' - The table!</li>
		...
		/**
		 * Sorting that is always applied to the table (i.test. prefixed in front of
		 * aaSorting).
		 * Note that this parameter will be set by the initialisation routine. To
		 * set a default use {@link DataTable.defaults}.
		 *  @type array
		...
		/**
		 * Callback functions array for every time a row is inserted (i.test. on a draw).
		 *  @type array
		 *  @default []
		 */
		"aoRowCallback": [],
		 ...
		 * (i.test. before filtering), Use fnRecordsTotal rather than
		 ...
		 * (i.test. after filtering). Use fnRecordsDisplay rather than
		 * this property to get the value of the number of records, regardless of
		 * the server-side processing setting.
		 *  @type boolean
		 *  @default 0
	...
	DataTable.ext = _ext = {
		 ...
		 *    // fourth column (i.test. the data[3] index) based on two input values
	...
	$.extend( DataTable.ext.type.detect, [
		...
		// test.g. Date.parse('55') (but not all, test.g. Date.parse('22')...).
		function ( d )
		{
	 ...
	 *  @event
	 *  @param {event} test jQuery event object
	 *  @param {object} o DataTables settings object {@link DataTable.models.oSettings}
	 *  @param {object} json JSON returned from the server
	 *
	 *  @example
	 *     // Use a custom property returned from the server in another DOM element
	 *     $('#table').dataTable().on('xhr.dt', function (test, settings, json) {
